...
\\
\psi_X(\alpha)=sup\{\psi_X(\alpha_n)\mid \exists{a_n}((a_0=0)\land \forall n((a_{n+1}>a_n)\land(\psi_X(\alpha_{n+1}>\psi_X(\alpha_n)))\land(sup{\alpha_n}=\alpha))\ ) \},isProjection(\alpha,X)\\
